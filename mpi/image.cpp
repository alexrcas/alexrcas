...
void saveImage(Image &image, const char *filename)
{
    ...
    for (int y = 0; y < height; y++)
        for (int x = 0; x < width; x++)
            imageFile[y][x] = image[y][x];

    imageFile.write(filename);
}

Image applyFilter(Image &image, Matrix &filter, int infLimitH, int supLimitH, int infLimitW, int supLimit)
{
    int height = image[0].size();
    int width = image[0].size();
    ...
    std::cout<< supLimitH << std::endl;
        for (int i = infLimitH; i < supLimitH; i++)
